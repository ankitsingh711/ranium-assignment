...
- **Google Books fallback link, not a second data source.** When a search fails, the error state shows a "Search Google Books instead" link that opens Google Books in a new tab with the same query — it does not fetch or merge Google Books data into the app. A real dual-source fallback (retrying the search against Google Books and tagging/routing results by source) was considered and explicitly scoped down: the assignment specifies Open Library as the data source, and a seamless fallback would require a `source` field threaded through `BookSummary`/`BookDetail` and source-aware detail routing — real complexity for a resilience feature against an outage outside the app's control. The link-out gives the same practical benefit (the user isn't stuck) without that complexity.
