...
- Automatic in-app fallback to the Google Books API when Open Library is unreachable, both for search and for a book's detail page — results are shown directly in the app (with a small "Google Books" badge/notice), not a link out; see Architecture and Tradeoffs
...
- [Google Books API](https://developers.google.com/books/docs/v1/using) — no API key required, automatic fallback when Open Library is unreachable
...
**API/data layer** (`useBooks.ts`) is the only place that talks to Open Library (and its fallback, Google Books). It maps `OpenLibrarySearchDoc` / `OpenLibraryWork` / `OpenLibraryEdition` / `OpenLibraryAuthor` / `GoogleBooksVolume` into `BookSummary` / `BookDetail`, so the rest of the app never sees raw API fields from either source. Search results come straight from `search.json`. Detail pages combine three calls — the work, its author, and its first edition (for publisher/page count, which the work endpoint doesn't reliably carry) — each independently wrapped so a missing author or edition doesn't fail the whole page.

**Fallback data source**: Open Library is the primary source everywhere. If a search or a detail fetch to Open Library fails or exceeds a 5s timeout, `useBooks.ts` automatically retries against the [Google Books API](https://developers.google.com/books/docs/v1/using) (no key required) and maps its response into the same `BookSummary`/`BookDetail` shape, tagged with `source: 'googlebooks'`. The UI shows a small inline notice on the search page and a "Google Books" badge on affected cards, so it's never silently swapping data sources. Because the two APIs use incompatible id schemes, Google Books results get a `gb_`-prefixed `id` (e.g. `gb_zyTCAlFPjgYC`) so `/books/{id}` knows which API to re-query on the detail page; Open Library ids are left exactly as-is (unprefixed), so this is fully backwards compatible with existing shortlist entries and bookmarked URLs from before this fallback existed.
...
- **Google Books fallback is a deliberate scope addition beyond the assignment brief**, which specifies Open Library as the data source. Added because Open Library had a real, extended outage during development (see Unfinished / Known Issues) and a silent dead end felt worse than a documented, explained exception. It's implemented as a genuine second data source (not a link-out) specifically because a link-out stops being useful once you've already told the user the primary source is down — worth being ready to justify that scope call in review.
- **The fallback only covers the two calls the UI blocks on** (search, and the work/volume detail fetch) — it does not extend to the Open Library author/editions enrichment calls on the detail page, since those already fail independently and gracefully (missing author/publisher/pages, not a page-level error).
- **Google Books' unauthenticated tier has a shared daily quota** and can itself return `429 Quota exceeded` — observed directly during development. The fallback's own failure path (a clear error message, no crash) is what runs in that case; there's no third source to fall back to further.
...
- **What was changed / rejected**:
  ...
  - The Google Books fallback went through two iterations at the developer's direction: first a link-out ("Search Google Books instead", opens in a new tab), then replaced with the current in-app fallback (real second data source, results rendered inside the app) once the developer decided a link-out wasn't good enough during a real Open Library outage. The link-out code (an `ErrorState` `fallbackHref` prop, a `googleBooksSearchUrl` util) was fully removed rather than left dead alongside the new implementation.
